...
test.describe.only("Career Opportunities Page Tests", () => {
  ...
  test("Check job listings are visible", async ({ page }) => {
    ...
    const jobList = page.locator('#VacanciesDIVNew div').filter({ hasText: 'Furniture Sales Executive IT' }).nth(1) // Replace with actual selector
  ...
  test('Apply for IT Operations Assistant', async ({ page }) => {
    const careerLink = page.getByRole('link', { name: 'Avatar Career Opportunities' });
